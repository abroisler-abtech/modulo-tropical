...
def exibir_metricas_detalhadas(df, col_qtd, col_uni, col_rota, col_empresa, col_req, col_prod, titulo=""):
    ...
    # --- NOVO GRUPO: ESTIMATIVA DE CAIXAS ---
    st.markdown("##### 📦 Estimativa de Caixas e Embalagens de Separação")
    
    # Separação de Ovos Comum vs PVC
    ...
    k1.markdown(f'<div class="card-verde"><div class="card-titulo">CX Peso (20kg)</div><div class="card-valor">{cx_kg} cx</div></div>', unsafe_allow_html=True)
    k2.markdown(f'<div class="card-verde"><div class="card-titulo">CX Und (180un)</div><div class="card-valor">{cx_un} cx</div></div>', unsafe_allow_html=True)
    k3.markdown(f'<div class="card-verde"><div class="card-titulo">Ovo PVC (10bdj)</div><div class="card-valor">{cx_ovo_pvc} cx ({int(bj_pvc)} bj)</div></div>', unsafe_allow_html=True)
    k4.markdown(f'<div class="card-verde"><div class="card-titulo">Ovo Comum (12bdj)</div><div class="card-valor">{cx_ovo_comum} cx ({int(bj_comum)} bj)</div></div>', unsafe_allow_html=True)
    k5.markdown(f'<div class="card-verde"><div class="card-titulo">CX Outros (20vol)</div><div class="card-valor">{cx_outros} cx</div></div>', unsafe_allow_html=True)
    k6.markdown(f'<div class="card-verde"><div class="card-titulo">Total de Caixas</div><div class="card-valor">{cx_total_geral} cx</div></div>', unsafe_allow_html=True)
...
if modulo == "📋 Separação do Dia":
    ...
    if uploaded_file is not None:
        try:
            ...
            # Botão de salvar no Supabase reativado e em destaque
